          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <p className="text-2xl font-bold mt-2">
                {paymentDetails?.reference || "Not available"}
              </p>
            </div>
            <div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {order.items?.map((item: any) => {
              ...
              return (
                <div key={item.itemId} className="app-card p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-center gap-4 min-w-0">
                      {imageSrc ? (
                        <img
                          src={imageSrc}
                          alt={catalogItem?.name || `Item ${item.itemId}`}
                          className="w-20 h-20 object-contain rounded shrink-0"
                        />
                      ) : (
                        <div className="w-20 h-20 rounded bg-muted-surface shrink-0" />
                      )}
